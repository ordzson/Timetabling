...
Motor: clases y exámenes usan el mismo modelo. `schedule_plan.schedule_type`
decide el modo.

## Principios

- PostgreSQL es la fuente de verdad.
- La base de datos guarda catálogos, planes, corridas, asignaciones, conflictos y auditoría.
- El motor valida solapes complejos. La base de datos protege claves foráneas (FK), unicidad básica y restricciones CHECK.
- Área común se modela como una sola `schedule_session` con varias cohortes
  compatibles por jornada y pensum.
- Disponibilidad docente queda normalizada en `teacher_availability`; preferencias y
  instantáneas flexibles van en JSONB.
- No crear `exam_plan` separado por ahora. Si se necesita para interfaz, usar una vista.

## DER resumido

```mermaid
erDiagram
  app_user }o--|| teacher : "teacher_id"
  app_user }o--|| cohort : "cohorte del estudiante"

  career ||--o{ curriculum : "tiene"
  career ||--o{ cohort : "tiene"
  curriculum ||--o{ curriculum_course : "contiene"
  course ||--o{ curriculum_course : "aparece_en"
  curriculum ||--o{ cohort : "usado_por"
  journey ||--o{ cohort : "asiste_a"
  journey ||--o{ time_block : "tiene"
  journey ||--o{ fixed_break : "bloquea"

  teacher ||--o{ teacher_course : "puede_impartir"
  course ||--o{ teacher_course : "impartido_por"
  teacher ||--o{ teacher_availability : "disponible"

  room ||--o{ room_resource : "tiene"
  course ||--o{ course_required_resource : "requiere"

  course ||--o{ common_area_rule : "regla_compartida"
  common_area_rule ||--o{ common_area_career : "incluye"
  career ||--o{ common_area_career : "participa"
  curriculum ||--o{ common_area_career : "usa_pensum"
  journey ||--o{ common_area_rule : "misma_jornada"

  schedule_plan ||--o{ schedule_session_group : "agrupa"
  schedule_session_group ||--o{ schedule_session : "mantiene_docente"
  schedule_plan ||--o{ schedule_session : "requiere"
  schedule_session ||--o{ schedule_session_cohort : "afecta"
  cohort ||--o{ schedule_session_cohort : "asiste"
  schedule_plan ||--o{ schedule_run : "genera"
  schedule_run ||--o{ schedule_assignment : "produce"
  schedule_session ||--o{ schedule_assignment : "asignada_como"
  time_block ||--o{ schedule_assignment : "inicia_en"
  teacher ||--o{ schedule_assignment : "asignado"
  room ||--o{ schedule_assignment : "asignada"

  schedule_run ||--o{ schedule_violation : "reporta"
  schedule_plan ||--o{ pre_validation_issue : "reporta"
  schedule_plan ||--o{ section_suggestion : "sugiere"
  schedule_plan ||--o{ manual_edit : "edita"
  schedule_assignment ||--o{ substitution_event : "sustituida"
```
...
`common_area_rule` define que un curso se une físicamente para varias carreras
en una misma jornada. `common_area_career` amarra cada carrera a su pensum.
La sesión concreta vive después en `schedule_session`.
...
## Validaciones, conflictos y sugerencias
...
## Edición manual y sustituciones
...
## Reglas que NO van en base de datos

- Solape por cohorte.
- Caminata entre aulas.
- Ventanas muertas.
- Contiguidad de bloques del mismo curso.
- Balance de carga docente.
- Reparación LNS.

Estas reglas viven en el dominio/motor. La base de datos solo conserva datos,
resultado y auditoría reproducible.

## Vista opcional para exámenes
